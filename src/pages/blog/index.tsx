...
const Blog: NextPage<ArticlesProps> = ({ articles }) => {
  ...
  return (
    <Layout title="Blog">
      <>
        <NextSeo
          title="Blog -- Mohammad Riswanto"
          description="I write blog, Share a bit of my knowladge about web developement relative topics"
          openGraph={{
            type: "website",
            title: "Blog -- Mohammad Riswanto",
            description:
              "I write blog, Share a bit of my knowladge about web developement relative topics",
        ...
        <header className="mt-5">
          ...
          <p className="text-black dark:text-light/70">
            I write blog, Share a bit of my knowledge about web development related topics, Enjoy it
            fellas!
          </p>
        </header>
        ...
        <section className="grid gap-10 mt-12" aria-label="article-wrapper">
          {Articles.length === 0 ? (
            <span className="text-base text-black dark:text-white">No article :(</span>
          ) : (
            Articles.map((article, i) => (
              <Link key={i} href={"/blog/" + article.slug}>
                <article aria-label="card-wrapper" role={"button"} tabIndex={0}>
                  <div className="flex items-center text-xs gap-4 mb-1">
                    <p>{article.created}</p>
                    <p>{article.tag}</p>
                  </div>
                  <h2 className="font-extrabold text-xl hover:underline">{article.title}</h2>
                  <p className="text-black dark:text-light/70 font-light text-sm md:text-base mt-1">
                    {article.highlight}
                  </p>
                </article>
              </Link>
